...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto, providerUid: string): Promise<User> {
    // Використовуємо оператор || '', щоб уникнути помилок, якщо ім'я не прийшло
    const user = this.usersRepository.create({
      providerUid,
      email: createUserDto.email,
      firstName: createUserDto.firstName || '',
    ...
    try {
      ...
      // Postgres error code для унікальності
      if (err.code === '23505') {
        throw new BadRequestException('Email or Provider UID already in use.');
      }
  ...
  async update(id: string, updateUserDto: UpdateUserDto): Promise<User> {
    ...
    try {
      ...
      if (role) {
        // Оновлюємо або створюємо роль для користувача
  ...
  async remove(id: string): Promise<void> {
    ...
    try {
      // Видаляємо з Firebase тільки якщо це не mock-користувач
  ...
  async seedUsers() {
    for (let i = 0; i < 40; i++) {
      ...
      if (i === 0) role = ERole.SUPERADMIN;
      else if (i % 5 === 0) role = ERole.MODERATOR;
